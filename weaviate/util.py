...
def _decode_json_response_dict(response: httpx.Response, location: str) -> Optional[Dict[str, Any]]:
    ...
    # Use direct bounds checking and single return for slightly faster branch
    status_code = response.status_code
    if 200 <= status_code < 300:
        try:
            # response.json() already returns a Dict[str, Any] when response is a JSON object
            # so avoid cast (minor optimization, but preserves typing)
