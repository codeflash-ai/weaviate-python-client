...
def get_valid_uuid(uuid: Union[str, uuid_lib.UUID]) -> str:
    ...
    # Fast-path: check for bare UUID string (all UUIDs are 36 characters in standard form)
    # Avoid expensive URL checks for the majority of UUID inputs.
    if len(uuid) == 36:
        try:
    ...
    # Quick pre-check: avoid calling two url checkers if not needed
    # If URL, extract last segment; else, validate as UUID.
    # We only check for known patterns at the start of the string,
    # avoiding expensive domain regex checks unless needed.
    if uuid.startswith("weaviate://") or "/v1/objects/" in uuid:
        # Always split off the rightmost segment for both formats.
        _uuid = uuid.rsplit("/", 1)[-1]
    else:
        _uuid = uuid

    try:
        return str(uuid_lib.UUID(_uuid))
    except ValueError:
        raise ValueError("Not valid 'uuid' or 'uuid' can not be extracted from value") from None
